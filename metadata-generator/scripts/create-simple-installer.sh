...
echo -e "${BLUE}=== MetadataGenerator Simple Installer Creator ===${NC}"
echo -e "${YELLOW}This creates a DMG with no external dependencies required${NC}"
...
# Build the release version (GUI only, no CLI)
...
DMG_NAME="${APP_NAME}-Simple-Installer"
echo -e "${YELLOW}Creating simple DMG installer...${NC}"
...
create-dmg \
  --volname "${APP_NAME} Simple Installer" \
  ${VOLUME_ICON} \
  --window-pos 200 120 \
  --window-size 800 400 \
  --icon-size 100 \
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Simple DMG installer created successfully: ${DMG_NAME}.dmg${NC}"
    ...
    echo -e "${GREEN}=== Simple installer ready! ===${NC}"
    echo -e "${BLUE}This DMG contains a self-contained app with no external dependencies${NC}"
    echo -e "${YELLOW}Note: Video/audio analysis is simplified (no resolution/duration)${NC}"
    echo -e "${GREEN}✓ No Xcode CLI tools required${NC}"
    echo -e "${GREEN}✓ No FFmpeg required${NC}"
    echo -e "${GREEN}✓ No Homebrew required${NC}"
else
    echo -e "${RED}✗ Failed to create simple DMG installer${NC}"
    exit 1
fi
...
echo -e "${BLUE}Your simple installer is ready: ${DMG_NAME}.dmg${NC}"
echo -e "${YELLOW}This installer requires no additional dependencies!${NC}" 